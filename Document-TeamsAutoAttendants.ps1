...
# Fail fast on unhandled errors so failures are visible and the script stops cleanly.
...
# Global caches to avoid repeated lookups (performance + reduces service throttling).
# These caches are safe because the script is read-only and runs in a single PowerShell session.
...
function Build-TargetIndex {
    ...
    # Load all Call Queues; map CQ by Id and ResourceAccount -> CQ (best-effort).
    $allCqs = @()
    ...
    # Index the provided Auto Attendants (names + RA bindings) for cross-resolution.
...
function Get-IdFromCallTarget {
    ...
    # Prefer canonical Id when present.
    if ($CallTarget.PSObject.Properties['Id']) {
        $v = [string]$CallTarget.Id
        if ($v) { return $v }
    }

    # Common property names observed across Teams voice objects (best-effort).
    ...
    # Last resort: scan all properties for something that looks like an identifier.
...
function Normalize-TargetType {
    param([string]$Type)
    # Normalises known variants emitted by Teams objects into a smaller, report-friendly set.
    switch -Regex ($Type) {
...
function Get-HumanTypeLabel {
    ...
    # Human-friendly label used within bracketed audit details in the HTML output.
...
function Resolve-CallTargetFriendly {
    <#
      .SYNOPSIS
      Resolves a Teams voice CallTarget object to a friendly display label.

      .DESCRIPTION
      Accepts a CallTarget-like object (commonly returned inside menus/call flows) and returns
      a consistent structure used across the report:
        - RawType: original Type value
        - Id: extracted identifier (best-effort)
        - NormType: normalised type (CallQueue/AutoAttendant/ResourceAccount/Group/User/Pstn/Unknown)
        - Friendly: resolved display label (best-effort)
        - AttachedType / AttachedName: when the target is a resource account that is attached to a CQ/AA

      .NOTES
      Resolution is best-effort and may fall back to IDs when lookups are not possible or commands are unavailable.
    #>
    param($CallTarget)
    ...
    # Small helper to keep optional lookups non-fatal.
    ...
    switch ($type) {
        ...
        'Pstn' {
            # For presentation, strip tel: when present.
        ...
        'ResourceAccount' {
            ...
            # If the RA is attached to a CQ/AA, prefer showing the attached object name in the report.
        ...
        default {
            # Final fallback: if the ID looks like an application instance, show that display name; else show raw ID.
            if ($id -and -not $script:_ResolveCache.RAs.ContainsKey($id)) {
                $ra = _try { Get-CsOnlineApplicationInstance -Identity $id -ErrorAction Stop }
...
function Resolve-AnyIdFriendly {
    <#
      ...
      .DESCRIPTION
      Used when CallTarget.Type is missing/incorrect or when the friendly name cannot be resolved from the
      typed CallTarget logic. Returns the same key set as Resolve-CallTargetFriendly.

      .RETURNS
      Hashtable with keys: RawType, Id, NormType, Friendly, AttachedType, AttachedName
    #>
    param([string]$Id)
...
function Format-TargetBracket {
    # Builds a consistent bracketed audit string, including raw type/id and RA attachment context (when applicable).
    param($Resolved)
...
function Get-PromptSummary {
    # Normalises a greeting prompt into a simple report label (TTS vs Audio vs None).
    param($Prompt)
...
function Get-GreetingsSummary {
    # Collapses one or more greetings into a <br/> separated summary for HTML output.
    param($Greetings)
...
function Get-MenuPromptsSummary {
    # Extracts the default menu prompt summary from DefaultCallFlow.Menu.Prompts (if present).
    param($Menu)
...
function Build-FriendlyBlockFromCallTarget {
    ...
    # Fallback resolution if friendly is blank or equals Id (common when Type is not helpful).
...
function Get-CallFlowModeSummary {
    <#
      Produces a compact HTML summary describing greeting + call handling mode (Menu/Disconnect/Redirect/etc.).
      The classification logic is designed to be deterministic and consistent across Default/AfterHours/Holiday flows.
    #>
    param($CallFlow)
...
function Get-FixedScheduleRangesText {
    # Formats Fixed schedules (e.g., Holidays) as "dd/MM/yyyy HH:mm-dd/MM/yyyy HH:mm; ..."
    param($Schedule)
...
function Get-WeeklyScheduleText {
    # Formats WeeklyRecurrentSchedule into a readable day-by-day list for HTML.
    param($Weekly)
...
function Get-BusinessHoursText {
    <#
      Returns the business-hours schedule as a weekly textual representation, if identifiable.
      Priority:
        1) BusinessHours association schedule
        2) Weekly schedules with a "business/work/working/hours" name hint
        3) First weekly schedule found
        4) Not configured
    #>
    param(
...
function Get-HolidayLines {
    # Builds HTML lines for each Holiday association including schedule date ranges, greeting, action, and target.
    param(
...
function Get-ActionAndTargetFromMenu {
    <#
      Extracts a representative Action + Target from a Menu. Designed for summary contexts (e.g., Holidays).
      Selection prioritises:
        1) Automatic + Transfer + CallTarget
        2) Automatic + CallTarget
        3) Any option with CallTarget
        4) First option
    #>
    param($Menu)
...
function Get-DefaultCallFlowCleanSummary {
    # Alternative summary (currently unused by main output); retained for future extensions.
    param($CallFlow)
    if (-not $CallFlow) { return "Not configured" }
...
function Get-DefaultMenuOptionsLines {
    # Produces HTML lines for each configured DTMF option including action and resolved target.
    param($Menu)
...
function Resolve-AuthorizedUsers {
    <#
      Resolves the AA AuthorizedUsers list into friendly labels, caching results to reduce repeated lookups.
      Prefers DisplayName, then UPN, then SIP, and finally raw ID as a fallback.
    #>
...
function New-ParamValueTableHtml {
    <#
      Builds a simple Parameter/Value HTML table for a single AA section.
      Values are expected to already be HTML-safe where required (HtmlEncode is applied to the Parameter name).
    #>
    param(
...
try {
    ...
    # Cache used for authorised user resolution across all AAs.
    $authUserCache = @{}

    # Collection of per-AA HTML sections to be concatenated into the final report.
    $sections = @()
    ...
    # ----------------------------- HTML Styles -----------------------------
    # Report styling is embedded to keep output portable (single HTML file).
    $style = @"
<style>
...
"@ | Out-File -FilePath $OutputPath -Encoding UTF8
    ...
    # Preserve the original terminating behaviour while still emitting a helpful error record.
    Write-Error $_
    throw
}